...
def chancetobuy(fday,td):
    ...
    summary = summary_data_from_transaction_data(tdd1,
                                             ...
                                             observation_period_end='2015-11-01')
...
print(lastprod)
recent_custdate=lastprod[['day_key','Cust_id']]
...
print(recent_custdate)
recent_custdate=recent_custdate.drop_duplicates('Cust_id')
...
#%%   
def cluster(custid,ctb,recent_custdate,lastprod,cleandata):
    print(cleandata)
    final= cleandata
    def prodfetch(custid):
        for r in range(0, len(recent_custdate)):
            if recent_custdate.iloc[r]['Cust_id'] == custid:
                for s in range(0, len(lastprod)):
                    if recent_custdate.iloc[r]['Cust_id']== lastprod.iloc[s]['Cust_id'] and recent_custdate.iloc[r]['day_key'] == lastprod.iloc[s]['day_key']:
...
custid=recent_custdate['Cust_id'].tolist()
print(custid)  
cluster(custid,ctb,recent_custdate,lastprod,cleandata)
